...
- [x] В `cmd/server/main.go` после инициализации TVDB клиента (строка 62), запустить `api.SyncUnsyncedSeries(db, tvdbClient)` в фоновой горутине
- [x] Обернуть в `if tvdbClient != nil`
- [x] Запустить тесты
- [x] Собрать проект `go build ./...`

### Task 4: Финальная проверка
- [x] Запустить полный тестовый набор
- [x] Запустить линтер — все проблемы должны быть исправлены
- [x] Проверить что `go vet ./...` проходит
